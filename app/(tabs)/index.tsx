...
import { searchProduct, addFavorite } from '@/lib/api';
...
export default function HomeScreen() {
  ...
  const pickImageFrom = useCallback(async (source: 'camera' | 'library') => {
    const permission =
      source === 'camera'
  ...
  const handlePickImage = useCallback(() => {
    Alert.alert('Seleziona immagine', 'Da dove vuoi prendere la foto?', [
      { text: 'Fotocamera', onPress: () => pickImageFrom('camera') },
      { text: 'Libreria', onPress: () => pickImageFrom('library') },
  ...
  const handleImageSearch = useCallback(() => {
    ...
    Alert.alert(
      'Funzione in arrivo',
      'La ricerca da foto richiede ancora un aggiornamento del backend per riconoscere il prodotto. Per ora usa la ricerca per testo o link.'
    );
  }, [imageUri]);
  ...
  const handleAddFavorite = useCallback(async () => {
    ...
    try {
      ...
      if (!user) {
        Alert.alert('Accesso richiesto', 'Devi accedere per salvare i preferiti.');
        return;
      }
      await addFavorite({
        user_id: user.id,
        query: query.trim(),
        search_type: mode,
        last_best_price: result.bestPrice,
      });
      setFavoriteAdded(true);
      Alert.alert('Fatto', 'Prodotto aggiunto ai preferiti.');
    } catch (err) {
      console.error('Errore salvataggio preferito:', err);
      Alert.alert('Errore', 'Impossibile salvare il preferito. Riprova più tardi.');
    } finally {
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1, marginLeft: Spacing.md }}>
            <Text style={[styles.appName, { color: theme.textPrimary }]}>TRUTH</Text>
            <Text style={[styles.appTagline, { color: theme.textSecondary }]}>{t('home_subtitle')}</Text>
          </View>
        </View>

        {/* Unified Tab Selector */}
        <View style={styles.modeSelectorWrapper}>
          <View style={[styles.modeSelectorContainer, { backgroundColor: theme.surfaceAlt, borderColor: theme.border }]}>
            {modes.map((m, idx) => {
              const Icon = m.icon;
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => { setMode(m.key); setQuery(''); setResult(null); setHasSearched(false); setErrorMessage(null); setImageUri(null); setFavoriteAdded(false); }}
                  activeOpacity={0.7}
                >
                  <Icon
                    size={18}
                    color={active ? theme.colors.primary[500] : theme.textTertiary}
                    strokeWidth={2.2}
                  />
                  <Text style={[styles.modeLabel, { color: active ? theme.colors.primary[500] : theme.textTertiary }, active && { fontWeight: '700' }]}>
                    {m.label}
                  </Text>
                </TouchableOpacity>
              );
        ...
        <View style={[styles.searchCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          {mode === 'image' ? (
            <View>
              <TouchableOpacity
                style={[styles.imageUploadArea, { backgroundColor: theme.surfaceAlt, borderRadius: Radius.lg }]}
                activeOpacity={0.8}
              ...
              >
                {imageUri ? (
                  ...
                  <View style={[styles.imageUploadIconCircle, { backgroundColor: theme.colors.primary[50] }]}>
                    <Camera size={32} color={theme.colors.primary[500]} strokeWidth={1.8} />
                  </View>
                )}
                <Text style={[styles.imageUploadText, { color: theme.textSecondary }]}>
                  {imageUri ? 'Tocca per cambiare foto' : t('image_search_hint')}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleImageSearch} disabled={!imageUri} activeOpacity={0.85}>
                <LinearGradient
                  colors={
                    imageUri
                      ? theme.mode === 'dark' ? ['#3b6bff', '#ff5ab0'] : ['#2b50f0', '#7c3aed']
                      : [theme.colors.neutral[300], theme.colors.neutral[300]]
                  }
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                  style={styles.gradientButton}
                >
                  <Text style={styles.gradientButtonText}>{t('search_button')}</Text>
                </LinearGradient>
              </TouchableOpacity>
            </View>
          ) : (
            <>
              <View style={[styles.searchInputRow, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}>
                {mode === 'url' ? (
                  <LinkIcon size={20} color={theme.textTertiary} strokeWidth={2} />
                ) : (
                  <Search size={20} color={theme.textTertiary} strokeWidth={2} />
                )}
                <TextInput
                  style={[styles.searchInput, { color: theme.textPrimary }]}
                  placeholder={mode === 'url' ? t('paste_url_placeholder') : t('search_placeholder')}
              ...
              <LinearGradient
                colors={query.trim() && !loading
                  ? (theme.mode === 'dark' ? ['#3b6bff', '#9b5cff'] : ['#2b50f0', '#7c3aed'])
                  : [theme.colors.neutral[300], theme.colors.neutral[300]]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={[
                  ...
                  query.trim() && !loading && {
                    shadowColor: theme.mode === 'dark' ? '#3b6bff' : '#2b50f0',
              ...
              >
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Search size={18} color="#fff" strokeWidth={2.5} style={{ marginRight: 8 }} />
        ...
        {/* Quota indicator */}
        <View style={[styles.quotaBar, { backgroundColor: theme.surfaceAlt }]}>
          ...
          <TouchableOpacity style={[styles.quotaUpgrade, { backgroundColor: theme.colors.accent[50] }]}>
            <Sparkles size={13} color={theme.colors.accent[600]} strokeWidth={2} />
            <Text style={[styles.quotaUpgradeText, { color: theme.colors.accent[600] }]}>{t('upgrade_now')}</Text>
          </TouchableOpacity>
        </View>
        ...
        {result && !loading && !errorMessage && (
          <View style={styles.resultsSection}>
            ...
            <View style={[styles.priceSummaryCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              <Text style={[styles.resultQuery, { color: theme.textSecondary }]} numberOfLines={1}>{query}</Text>
              <View style={styles.priceSummaryRow}>
                <View style={[styles.priceBox, { backgroundColor: theme.colors.success[50], borderColor: theme.colors.success[400] }]}>
                  <View style={styles.priceBoxHeader}>
                    <TrendingDown size={16} color={theme.colors.success[500]} strokeWidth={2.5} />
                    <Text style={[styles.priceBoxLabel, { color: theme.textSecondary }]}>{t('best_price')}</Text>
                  </View>
                  <Text style={[styles.bestPriceValue, { color: theme.colors.success[500] }]}>
                ...
                <View style={[styles.priceBox, { backgroundColor: theme.surfaceAlt, borderColor: theme.border }]}>
                  ...
                  <Text style={[styles.avgPriceValue, { color: theme.textPrimary }]}>
                    {result.averagePrice.toFixed(2)} <Text style={styles.currencySymbol}>€</Text>
                  </Text>
                </View>
              </View>
            </View>
                  ...
                  <View style={styles.merchantCardRight}>
                    ...
                    {idx === 0 && (
                      <View style={styles.bestDealTag}>
                        <Text style={styles.bestDealTagText}>MIGLIORE</Text>
                      </View>
                    )}
            ...
            <TouchableOpacity
              style={[styles.favButton, { backgroundColor: theme.colors.accent[50], borderColor: theme.colors.accent[200] }]}
              activeOpacity={0.8}
              onPress={handleAddFavorite}
              disabled={addingFavorite || favoriteAdded}
            >
              {addingFavorite ? (
                <ActivityIndicator size="small" color={theme.colors.accent[500]} />
              ) : (
                <Star
                  size={18}
                  color={theme.colors.accent[500]}
                  strokeWidth={2}
                  fill={favoriteAdded ? theme.colors.accent[500] : 'none'}
                />
              )}
              <Text style={[styles.favButtonText, { color: theme.colors.accent[600] }]}>
                {favoriteAdded ? 'Aggiunto ai preferiti' : t('add_to_favorites')}
              </Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {!result && !loading && !errorMessage && hasSearched && (
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyTitle, { color: theme.textPrimary }]}>{t('no_results')}</Text>
          </View>
        )}
